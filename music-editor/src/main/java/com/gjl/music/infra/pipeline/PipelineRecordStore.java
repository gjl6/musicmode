 ...
 * <h3>核心策略</h3>
 * <ul>
 *   <li><b>Redis</b> — 缓存最新 / 活跃 Top 100 条管道记录（ZSET 自动裁剪），快速响应前端首页查询</li>
 *   <li><b>DB</b> — 全量存量持久化，所有数据最终入库，永久保留</li>
 * </ul>
 *
 * <h3>读写路径</h3>
 * <ul>
 *   <li><b>写入</b>：同步写 DB + 写 Redis ZSET/Hash，ZSET 自动裁剪超出 100 的旧数据</li>
 *   <li><b>终态</b>：变为 COMPLETED/FAILED/CANCELLED 时从 Redis 移除（DB 永久保留）</li>
 *   <li><b>首页查询（≤100 条）</b>：全走 Redis ZREVRANGE + 批量 HGETALL</li>
 *   <li><b>历史查询（&gt;100 条）</b>：穿透查 DB</li>
 *   <li><b>缓存穿透防护</b>：DB 空结果缓存空标识（TTL=5min）</li>
 * </ul>
 *
 * <h3>Redis 数据结构</h3>
 * <pre>{@code
 *   pipeline:active            ZSET   score=createdAt, member=pipelineId  (max 100)
 *   pipeline:task:{id}         HASH   fields                             (与 ZSET 一一对应)
 * }</pre>
 *
 * <h3>数据一致性</h3>
 * <p>状态/进度变更时同时更新 DB + Redis（若在 Redis 中）。</p>
 */
@Slf4j
@Component
public class PipelineRecordStore {
    ...
    /** 新建任务。同步写 DB + 写 Redis ZSET/Hash，自动裁剪 Top 100。 */
    ...
    public void updateState(String pipelineId, PipelineState state) {
        if (redisEnabled) {
            try {
                ...
                log.warn("Redis 更新状态失败（忽略）: {} — pipeline={}", e.getMessage(), pipelineId);
            }
        }
    }

    /** 更新进度。DB 始终更新；Redis 仅当管道仍在缓存中时更新。 */
    public void updateProgress(String pipelineId, int totalFiles, int successFiles,
        ...
        if (redisEnabled) {
            try {
                ...
                log.warn("Redis 更新进度失败（忽略）: {} — pipeline={}", e.getMessage(), pipelineId);
            }
        }
    }
    ...
    /**
     * 列出管道任务（DB 全量 + Redis 活跃覆盖实时进度）。
     *
     * <p>DB 提供全量历史 + state 等静态字段；Redis 仅覆盖活跃管道的实时进度数字。
     * state 变更走 DB（同步写入），不依赖 Redis 加速。</p>
     */
    public List<PipelineTaskInfo> listAll() {
        ...
        // Redis 活跃覆盖：仅覆盖实时进度数字（state 走 DB）
        if (redisEnabled) {
            try {
                ...
                for (PipelineTaskInfo info : result) {
                    ...
                    if (live != null && !live.isEmpty()) {
                        // 终态不应该出现在 Redis 中 → 清理残留
                        if (TERMINAL_STATES.contains(info.getState())) {
                            redis.opsForZSet().remove(ZSET_ACTIVE, info.getPipelineId());
                            redis.delete(hashKey(info.getPipelineId()));
                            continue;
                        }
                        overlay(info, live);
                    }
    ...
    /**
     * 按 ID 查询单个。DB 为权威数据源，Redis 仅覆盖实时进度。
     * 若 DB 中不存在则返回 null，同时清理 Redis 残留。
     */
        ...
        if (task == null) {
            ...
            if (redisEnabled) {
                try {
                    ...
                    log.debug("Redis 清理残留失败: {}", pipelineId, e.getMessage());
                }
            }
            return null;
        }

        PipelineTaskInfo info = PipelineTaskInfo.from(task);
     ...
     * 加载非终态管道（崩溃恢复用）。
     *
     * <p>DB 是权威数据源，Redis 仅用于覆盖实时进度。
     * 恢复时以 DB 为准，避免 Redis 中的"幽灵管道"（DB 已删除但 Redis 未清理）反复出现。</p>
     */
    public List<PipelineTaskInfo> loadNonTerminal() {
        // ★ DB 是权威数据源，先查 DB
        List<PipelineTask> tasks = db.loadNonTerminal();
        List<PipelineTaskInfo> dbResults = tasks.stream()
                .map(PipelineTaskInfo::from)
                .collect(Collectors.toList());

        if (dbResults.isEmpty()) {
            // DB 中没有非终态管道 → 清理 Redis 残留，避免下次启动再度出现幽灵管道
            if (redisEnabled) {
                try {
                    Set<String> redisIds = redis.opsForZSet().reverseRange(ZSET_ACTIVE, 0, -1);
                    if (redisIds != null && !redisIds.isEmpty()) {
                        long removed = redis.opsForZSet().remove(ZSET_ACTIVE, redisIds.toArray(new String[0]));
                        for (String id : redisIds) {
                            redis.delete(hashKey(id));
                        }
                        log.info("清理 Redis 残留管道: {} 条（DB 中已不存在）", removed);
                    }
                } catch (Exception e) {
                    log.debug("Redis 清理残留失败（忽略）: {}", e.getMessage());
                }
            }
            return dbResults;
        }

        // DB 有数据 → 用 Redis 活跃数据覆盖实时进度
        if (redisEnabled) {
            try {
                Map<String, Map<Object, Object>> activeSnapshots = readActiveFromRedisAsMap();
                for (PipelineTaskInfo info : dbResults) {
                    Map<Object, Object> live = activeSnapshots.get(info.getPipelineId());
                    if (live != null && !live.isEmpty()) {
                        overlay(info, live);
                    }
                ...
                // 清理 Redis 中 DB 不存在的幽灵管道
                Set<String> dbIds = dbResults.stream()
                        .map(PipelineTaskInfo::getPipelineId)
                        .collect(Collectors.toSet());
                Set<String> redisIds = redis.opsForZSet().reverseRange(ZSET_ACTIVE, 0, -1);
                if (redisIds != null) {
                    for (String rid : redisIds) {
                        if (!dbIds.contains(rid)) {
                            redis.opsForZSet().remove(ZSET_ACTIVE, rid);
                            redis.delete(hashKey(rid));
                            log.info("清理 Redis 幽灵管道: {}", rid);
                        }
                    }
                }
            } catch (Exception e) {
     ...
     * 清理过期管道。
     *
     * <p>DB 主清（全量历史，任何状态）；Redis 辅清。
     * Redis 清理现在处理两种场景：
     * <ol>
     *   <li>终态条目 — 从 ZSET + HASH 删除</li>
     *   <li>非终态但 DB 已不存在的"幽灵管道" — 从 ZSET + HASH 删除
     *       （这些是 DB 已清理但 Redis 因状态检查跳过而残留的）</li>
     * </ol>
     */
            ...
            try {
                ...
                if (oldIds != null && !oldIds.isEmpty()) {
                    ...
                    for (String id : oldIds) {
                        ...
                        // 非终态 → 检查 DB 是否还存在，不存在则清理幽灵
                        else if (db.findTaskById(id) == null) {
                            redis.opsForZSet().remove(ZSET_ACTIVE, id);
                            redis.delete(hashKey(id));
                            removed++;
                            log.info("清理 Redis 幽灵管道（DB 已删除）: {}", id);
                        }
                    }
                    if (removed > 0) {
                        log.info("cleanupExpired: Redis 清理 {} 条", removed);
                    }
                }
            } catch (Exception e) {
    ...
    /** 从 Redis 读取 Top 100 活跃管道（pipelineId → Hash fields 映射） */
    ...
    /** 从 Redis 读取 Top 100 活跃管道（列表形式，供 findById/loadNonTerminal 使用） */
    ...
    /** 用 Redis 实时数据覆盖 DB 静态记录（仅进度字段，state 由 DB 权威管理） */
