...
@Slf4j
@Service
public class MusicEditServiceImpl implements MusicEditService {
    ...
    public MusicEditServiceImpl(FileSystemModule fileSystem,
    ...
    public MusicMetadata saveFields(String rawPath, Map<String, Object> metaMap) {
        ...
        if (!file.exists() || !file.isFile()) {
            throw new IllegalArgumentException("文件不存??? " + path);
        }
        ...
        // 写入文件标签后同步更新数据库（Song + Artist + Album 全 MERGE）
        metadataPersister.persistBatchMergeArtistAlbum(
                List.of(Map.entry(file.toPath().toAbsolutePath().toString(), editedMeta)));
