<template>
  <div class="aep-root">
    ...
    <div class="aep-section">
      <div class="aep-section-title">{{ $t('albumManager.toolEnrichScope') }}</div>
      <n-radio-group v-model:value="scope" size="small">
        <n-radio-button value="all">
          {{ $t('albumManager.toolEnrichAllResults', { n: total }) }}
        </n-radio-button>
        <n-radio-button value="selected" :disabled="!selectedIds.length">
          {{ $t('albumManager.toolEnrichSelected', { n: selectedIds.length }) }}
        </n-radio-button>
      </n-radio-group>
    </div>
    ...
    <!-- 标签源 -->
    <div class="aep-section">
      <div class="aep-section-title">{{ $t('albumManager.toolEnrichProviders') }}</div>
      <n-select
        v-model:value="providers"
        :options="providerOptions"
        multiple
        :placeholder="$t('albumManager.toolEnrichProviders')"
...
import { submitAlbumPipeline, getAlbumProviders } from '@/api/editor/album-manage.js'
...
onMounted(async () => {
  try {
    const res = await getAlbumProviders()
    ...
    console.warn('[AlbumEnrich] 无法获取 provider 列表，使用默认', e)
...
async function doSubmit() {
  ...
  try {
    ...
    const res = await submitAlbumPipeline('album-enrich', sel)
    if (res?.pipelineId) {
      message.success(t('albumManager.pipelineSubmitted', { id: res.pipelineId }))
      emit('done')
    }
  } catch (e) {
    console.error('[AlbumEnrich] 提交失败:', e)
