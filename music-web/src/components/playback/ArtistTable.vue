    ...
    <table class="clay-table">
      <thead>
        <tr>
          <th class="col-idx">#</th>
          <th class="col-cv"></th>
          <th class="col-main">{{ t('artist.title') }}</th>
          <th class="col-num">{{ t('artist.albumCount', { count: 0 }).replace('0', '').trim() }}</th>
          <th class="col-num">{{ t('artist.songCount', { count: 0 }).replace('0', '').trim() }}</th>
          <th class="col-tag">{{ t('artist.colGender') }}</th>
          <th class="col-tag">{{ t('artist.colCountry') }}</th>
          <th class="col-act"></th>
        </tr>
      </thead>
      <tbody>
        <tr
          v-for="(row, idx) in artists"
          :key="row.id"
          class="clay-row"
        >
          ...
          <td class="col-num">{{ row.albumCount ?? '—' }}</td>

          <td class="col-num">{{ row.songCount ?? '—' }}</td>

          <td class="col-tag">
            <span v-if="genderLabel(row.gender)" class="badge badge-accent">{{ genderLabel(row.gender) }}</span>
            <span v-else class="dash">—</span>
          </td>

          <td class="col-tag">
            <span v-if="row.country" class="badge badge-subtle">{{ row.country }}</span>
            <span v-else class="dash">—</span>
          </td>
...
function goDetail(row) {
  if (row.id) router.push(`/player/artists/${row.id}`)
}
</script>
...
.col-tag   { width: 64px;  text-align: center; }
